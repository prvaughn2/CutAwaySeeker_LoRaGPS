...
void loop()
{
  ...
  SerialUSB.println("Sending -i am ready to receive a message.-");
...
//    SerialUSB.println((char*)buf);
     ...
     memcpy(&thisNodesManifest, &thisNodesManifest, sizeof(thisNodesManifest));

     for (int row = 0; row < NumofReadings; row = row + 1)
     {
        SerialUSB.print("pass# "); SerialUSB.println(row);
        SerialUSB.print("0sLAT=");  SerialUSB.println(thisNodesManifest[row].curr_Lat,6);
        SerialUSB.print("0sLONG="); SerialUSB.println(thisNodesManifest[row].curr_Lon,6);
     }
