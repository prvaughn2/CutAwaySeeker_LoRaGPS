...
//Variables for current location and time
struct information {
  int ID = node_ID ;
  double curr_Lat;
  double curr_Lon;
...
if (SerialUSB.available()) {      // If anything comes in Serial (USB),
  ...
  if (rf95.available()){
    ...
    if (rf95.recv(buf, &len)){
      ...
      //buffer = reinterpret_cast<uint8_t*>(&curr_info);
     
     // match_data((uint8_t *)&curr_info, sizeof(curr_info));
      
      //sprintf(buffer, "%s", curr_info);
      
      //TODO: pass buffer to radio
